...
describe("Provision and State Discovery refinement model", () => {
  it("keeps the supplied cross-team ownership, readiness rule, State, and Provision story discovery content", () => {
    ...
    expect(source).toContain("Define State Requirements");
    expect(source).toContain("Process & Govern State Data");
    expect(source).toContain("Consume Governed Output");
    expect(source).toContain("Define Provision Requirements");
    expect(source).toContain("Process & Govern Provision Data");
    expect(source).toContain("Continue Provision Workflow");
    expect(source).toContain("GOVERNED CHANGE LOOP");
    expect(source).toContain("Practitioner changes are returned through the governed DCT flow before the refreshed result is displayed in Roger.");
    expect(source).toContain('width: "100%", minWidth: 0');
    expect(source).not.toContain('minWidth: "960px"');
