...
describe("PI4 Post Pilot delivery", () => {
  ...
  it("moves the PI4 planning card from the landing page to the dedicated Post Pilot page", () => {
    ...
    expect(postPilot).toContain("DCT Sprint 2 Focus");
    expect(postPilot).toContain("High / Critical defects first");
    expect(postPilot).toContain("State · 3 Primary Goals");
    expect(postPilot).toContain("State Filing Footprint");
    expect(postPilot).toContain("Automated Apportionment & Payments");
    expect(postPilot).toContain("State Taxonomy / GoSystem Alignment");
    expect(postPilot).toContain("Sprint 2 goals — Pending Confirmation");
    expect(postPilot).toContain("Entity mapping requirements");
    expect(postPilot).toContain("Non-legal entity requirements");
    expect(postPilot).toContain("the three State goals apply to the current Sprint 2");
