...
describe("PI4 Post Pilot delivery", () => {
  ...
  it("moves the PI4 planning card from the landing page to the dedicated Post Pilot page", () => {
    ...
    expect(prototypeFlow).toContain("Client File to Roger — Federal, State &amp; Provision Process Flow");
    expect(prototypeFlow).toContain("One client-file journey: intake, governed preparation, Roger review, a single TDC decision, and controlled downstream delivery.");
    expect(prototypeFlow).toContain("Federal, State, and Provision are parallel practitioner workflows—not separate data pipelines.");
    expect(prototypeFlow).toContain("Five-phase executive process");
    expect(prototypeFlow).toContain("executive-client-file-to-roger-process-flow_0f8ee1d9.png");
    expect(prototypeFlow).toContain("Open full-size diagram");
    expect(prototypeFlow).toContain('minWidth: "2200px"');
    expect(prototypeFlow).toContain("Detailed technical contracts, APIs, field mappings, and integration rules remain governed");
    expect(prototypeFlow).not.toContain("Executive view:");
    expect(prototypeFlow).not.toContain("client-file-to-roger-federal-state-provision-process-flow_94710632.png");
    ...
    expect(postPilot.indexOf("<StateProvisionPrototypeFlow />")).toBeGreaterThan(postPilot.indexOf("State & Provision Prototypes"));
    expect(postPilot.indexOf("<StateProvisionPrototypeFlow />")).toBeLessThan(postPilot.indexOf("PI4 Pod Delivery & Data Review Process"));
