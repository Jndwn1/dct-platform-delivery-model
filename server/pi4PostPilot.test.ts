...
describe("PI4 Post Pilot delivery", () => {
  ...
  it("moves the PI4 planning card from the landing page to the dedicated Post Pilot page", () => {
    ...
    expect(prototypeFlow).toContain("Client files enter the Tax Portal, PDC establishes the financial record");
    expect(prototypeFlow).toContain("Federal and State practitioner journeys");
    expect(prototypeFlow).toContain("Provision included when applicable");
    expect(prototypeFlow).toContain("client-file-to-roger-federal-state-provision-process-flow_94710632.png");
    expect(prototypeFlow).toContain("Open full-size diagram");
    expect(prototypeFlow).toContain('minWidth: "1740px"');
    expect(prototypeFlow).toContain("Ownership boundary:");
    expect(prototypeFlow).toContain("TDC owns tax judgment, decisions, and lineage");
    expect(prototypeFlow).not.toContain("Roger End-to-End Operating Flow");
    expect(prototypeFlow).not.toContain("roger-post-pilot-end-to-end-executive-flow_f73fea32.png");
