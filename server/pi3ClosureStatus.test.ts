...
describe("MVP portfolio closure status model", () => {
  it("keeps Defect Tracking as the sole active workstream and records the four current-day closures", () => {
    const activeWorkstreams = BATCH_DELIVERY_RECORDS
      .filter(record => record.sourceStatusLabel === "Active")
      .map(record => record.id)
      .sort();

    expect(activeWorkstreams).toEqual(["DEFECT-TRACKING"]);
  ...
  it("derives current development from Defect Tracking only", () => {
    const activeStatusKeys = BATCH_DELIVERY_RECORDS
      .filter(record => record.sourceStatusLabel === "Active")
      .map(record => record.statusKey)
      .sort();

    expect(activeStatusKeys).toEqual(["defect-tracking"]);
    expect(deriveBatchMetrics(DEFAULT_STATUS).inDev).toBe(1);
    expect(deriveMvpMetrics(DEFAULT_STATUS).complete).toBe(26);
    expect(deriveMvpMetrics(DEFAULT_STATUS).inDev).toBe(1);
  });
  ...
  it("derives PI progress from the reconciled MVP delivery record population", () => {
    expect(derivePICompletion(DEFAULT_STATUS)).toMatchObject({
      ...
      pi3: { total: 12, complete: 11, pct: 92 },
    });
  ...
  it("rolls the four current-day closures into the reporting week and cumulative rollup", () => {
    ...
    expect(PI3_POST_BASELINE_CLOSURES).toHaveLength(14);
    expect(PI3_POST_BASELINE_CLOSURES.map(item => item.id)).toEqual([
      "B16", "B17", "B29", "B7", "B42", "B31-TDC", "B10", "B31-PDC", "PERFORMANCE-TESTING", "DCT-QA-WORKSTREAM", "B28", "B9A", "ENV-MANAGEMENT", "B45",
    ]);
    ...
    expect(closedThisWeek.map(item => item.id)).toEqual([
      "B7", "B42", "B31-TDC", "B10", "B31-PDC", "PERFORMANCE-TESTING", "DCT-QA-WORKSTREAM", "B28", "B9A", "ENV-MANAGEMENT", "B45",
    ]);
    expect(getPi3CumulativeCompleted()).toBe(25);
