...
export const PAGE_CONTEXT_REGISTRY: Record<string, PageContextEntry> = {
  ...
  // ─── HOME / DASHBOARD ────────────────────────────────────────────────────────
  "/": {
    pageTitle: "Executive Health",
    pageIcon: "🏠",
    description: "Executive operating-model dashboard — MVP and PI health, readiness, milestones, risks, decisions, and delivery direction",
  ...
  "/post-pilot": {
    ...
    features: ["PI4 Planning Visibility", "Planned PI4 Feature Metrics", "Roger to GoSystem State Calculation POC", "Minimum POC Input Package", "POC Meeting Transcript Source", "PI4 Tracker Navigation"],
    apis: [],
    ...
    batches: [],
    businessObjects: ["PI4 Planning Item", "Post Pilot Plan", "Minimum POC Input Package", "State Calculation Input Package", "State Calculation Review Package"],
    integrations: ["PI4 Sprint & Story Tracker", "Roger", "DCT", "Taxonomy", "IMS", "GoSystem"],
    lastUpdated: "2026-09-23",
    lastChange: "Distinguished the minimum DCT POC input package from the broader target State model, including representative-scenario boundaries, a detailed cross-system proof flow, an inbound/outbound proof path, and the BA inbound/outbound data-contract deliverable.",
  },
  ...
  "/tax-mapping": {
    ...
    pageIcon: "💰",
    description: "Tax field mapping — how financial data maps to IMS-ready tax fields for return engine delivery",
    features: ["Tax Field Mapping", "IMS Field Reference", "Mapping Validation", "Missing Field Detection"],
    ...
    screens: ["Tax Mapping Grid", "IMS Field Reference", "Validation Results", "Missing Field Report"],
    businessRules: ["All required IMS payload fields must have a mapping", "Missing required fields block IMS delivery", "Tax mappings are versioned by tax year"],
    batches: ["B4", "B6", "B9A"],
  ...
  "/discovery/roger-overview": {
    ...
    screens: ["Known Mapping Screen", "Classification Review", "Entity Resolution Panel", "Confidence Band Display", "Tax Object Map", "Audit Trail", "Decision Capture", "Validation Rules", "IMS Delivery Status"],
    businessRules: [
  ...
  "/discovery/gosystem": {
    ...
    pageIcon: "🔀",
    description: "IMS is the integration broker between DCT/Roger and all downstream return engines (GoSystem, CCH, OIT, future engines). DCT does not integrate directly with any return engine.",
    features: ["Engine Routing", "Payload Translation", "Inbound Data Retrieval via B9A Gateway", "Outbound Delivery", "Engine Lookup"],
    apis: ["GET /api/v1/ims/payload/{entityId}", "POST /api/v1/ims/deliver/{entityId}/{engine}", "GET /api/v1/ims/engine-lookup/{entityId}", "GET /api/v1/ims/delivery-status/{entityId}", "POST /api/v1/ims/inbound/{engine}/{entityId}"],
    stories: ["IMS Engine Routing", "IMS Payload Translation", "IMS Delivery Tracking", "IMS Engine Lookup", "IMS Inbound Handler"],
    screens: ["IMS Architecture Overview", "Engine Routing Rules", "Delivery Status", "API Reference"],
    businessRules: [
      "DCT does not integrate directly with any return engine — all routing is owned by IMS",
      "IMS retrieves governed data from TDC via the B9A Gateway (governed consumer)",
      "IMS owns all engine routing, payload translation, and delivery logic",
      "IMS does not compute tax or modify the governed payload it receives",
      "GoSystem is one of several return engines IMS may route to (CCH, OIT, future engines also supported)",
      "B28 delivers TDC provision reference data — it does NOT export directly to GoSystem",
    ],
    batches: ["B9A", "B16", "B28"],
  ...
  "/discovery/ecosystem": {
    ...
    pageIcon: "🌐",
    description: "Five-component DCT ecosystem — PDC, TDC, Orchestrator, Roger, and IMS (Integration & Management System)",
    ...
    businessRules: ["PDC owns financial data", "TDC owns tax decisions", "Orchestrator is stateless AI", "Roger is read-only consumer", "IMS is the integration broker to all return engines — DCT does not connect directly to GoSystem"],
    batches: ["All batches (reference)"],
  ...
  "/discovery/platform-responsibilities": {
    ...
    businessRules: ["PDC: financial data ingestion and normalization only", "TDC: tax decisions and immutable records only", "Orchestrator: AI execution only — no data ownership", "Roger: read-only practitioner interface", "IMS: integration broker to all return engines — no direct TDC → GoSystem connection"],
    batches: ["All batches (reference)"],
    businessObjects: ["PDC", "TDC", "Orchestrator", "Roger", "IMS"],
    integrations: ["PDC", "TDC", "Orchestrator", "Roger", "IMS"],
    lastUpdated: "2026-07-16",
  ...
  "/discovery/data-flow": {
    ...
    businessRules: ["Data flows in one direction: ERP → PDC → TDC → Roger / IMS → Return Engine", "Roger decisions flow back to TDC via Decision API", "IMS retrieves governed data from TDC via B9A Gateway and routes to return engines", "No direct TDC → GoSystem connection — all return engine delivery goes through IMS"],
    batches: ["B1", "B2", "B3", "B4", "B5"],
  ...
  "/onboarding": {
    ...
    businessRules: [
      ...
      "IMS is the integration broker — DCT does not connect directly to GoSystem or any return engine",
  ...
  // ─── PHASE 1 OPERATING-MODEL WORKSPACES ──────────────────────────────────────
  "/workspace/delivery": {
    pageTitle: "Delivery Management",
    pageIcon: "▤",
    description: "Operating workspace that links existing calendar, pipeline, batch, assurance, and controlled delivery-management capabilities without duplicating delivery data.",
    features: ["Delivery Calendar", "Current Pipeline", "Batch Detail", "Delivery Assurance", "Control Panel Access"],
    apis: ["Governed delivery model", "Control Panel / ADO-derived delivery status"],
    stories: ["Plan Batch Delivery", "Review Delivery Assurance", "Trace Batch Status"],
    screens: ["Delivery Management Hub"],
    businessRules: ["The workspace links to existing delivery capabilities", "Current delivery status remains governed by the delivery model", "Batch detail routes remain dynamic and preserved"],
    batches: ["All governed delivery batches"],
    businessObjects: ["Batch", "Gate", "PI", "Release Candidate"],
    integrations: ["BatchStatusContext", "Control Panel", "Ask Buddy"],
    lastUpdated: "2026-08-20",
    lastChange: "Phase 1 non-destructive delivery workspace hub added.",
  },
  "/workspace/roger": {
    pageTitle: "Product & Roger Readiness",
    pageIcon: "R",
    description: "Operating workspace linking existing Roger screen readiness, UI mapping, API readiness, consumer decisions, and confirmed gap evidence.",
    features: ["Screen Readiness", "UI Data Mapping", "API Readiness", "Consumer Decisions"],
    apis: ["Roger API Evolution", "Registered API documentation"],
    stories: ["Review Roger Readiness", "Map Screen to API", "Track Consumer Decisions"],
    screens: ["Product & Roger Readiness Hub"],
    businessRules: ["The 18-screen QA Registry remains the authoritative screen inventory", "Roger UI Data Mapping consumes the shared screen inventory", "No readiness dataset is duplicated in this hub"],
    batches: ["B5", "B6", "B7", "B9", "B9A", "B28", "B42"],
    businessObjects: ["RogerScreen", "MappingDecision", "ConsumerDecision", "ApiContract"],
    integrations: ["Roger QA Registry", "Roger UI Data Mapping", "Ask Buddy"],
    lastUpdated: "2026-08-20",
    lastChange: "Phase 1 non-destructive Roger readiness workspace hub added.",
  },
  "/workspace/discovery": {
    pageTitle: "Discovery & BA Workspace",
    pageIcon: "⌕",
    description: "Operating workspace linking the existing BA flow and specialized Prior Year, Master Data, traceability, and dependency evidence.",
    features: ["Requirement Discovery", "Readiness Checklist", "Story Builder", "Prior Year Workspace", "Master Data Governance"],
    apis: ["Ask Buddy evidence lookup", "Master Data artifact registry", "Data Mapping Assistant"],
    stories: ["Discover Requirement", "Confirm Readiness", "Create Story", "Review Prior Year Evidence"],
    screens: ["Discovery & BA Workspace Hub"],
    businessRules: ["Requirement Discovery → Readiness → Story is the standard BA flow", "Prior Year and Master Data evidence remains governed by registered artifacts", "No source facts are copied into this hub"],
    batches: ["B9A", "B28", "B31", "B42"],
    businessObjects: ["Requirement", "Story", "MappingArtifact", "PriorYearField", "MasterDataArtifact"],
    integrations: ["Discovery Center", "Ask Buddy", "Data Mapping Assistant"],
    lastUpdated: "2026-08-20",
    lastChange: "Phase 1 non-destructive Discovery and BA workspace hub added.",
  },
  "/workspace/architecture": {
    pageTitle: "Architecture & Governance",
    pageIcon: "⬡",
    description: "Operating workspace linking existing architecture primer, platform domains, architecture views, data governance, mapping decisioning, ADRs, and lineage evidence.",
    features: ["Architecture Primer", "Platform Domains", "Architecture Views", "Data Governance", "Mapping & Decisioning", "ADRs & Lineage"],
    apis: ["Registered API documentation", "Architecture artifact references"],
    stories: ["Review Architecture", "Trace Source of Truth", "Review Decision"],
    screens: ["Architecture & Governance Hub"],
    businessRules: ["Approved architecture and ADRs govern current-state answers", "Historical decisions require an explicit status", "Advanced technical views remain route-accessible"],
    batches: ["FC", "B2", "B2A", "B3", "B9A", "B16", "B42"],
    businessObjects: ["ArchitectureDecision", "LineageRecord", "DataModel", "Taxonomy", "ApiContract"],
    integrations: ["Architecture views", "Data Governance", "Ask Buddy"],
    lastUpdated: "2026-08-20",
    lastChange: "Phase 1 non-destructive Architecture and Governance workspace hub added.",
  },
  "/workspace/quality": {
    pageTitle: "QA / UAT / Deployment",
    pageIcon: "✓",
    description: "Operating workspace linking existing QA Registry, QA release evidence, UAT workflow, deployment history, and known-issue evidence.",
    features: ["QA Registry", "QA Release Evidence", "UAT Readiness", "Deployment History", "Known Issues"],
    apis: ["Deployment Registry", "QA Deployment Registry", "UAT Testing"],
    stories: ["Review QA Readiness", "Review Release Evidence", "Track UAT", "Trace Deployment"],
    screens: ["QA / UAT / Deployment Hub"],
    businessRules: ["QA Registry is the authoritative Roger screen-readiness source", "UAT workflow capability is distinguished from populated UAT evidence", "Deployment history remains preserved in the Deployment Registry"],
    batches: ["B16", "B17", "B28", "B29", "B31", "B42"],
    businessObjects: ["QaDeployment", "QaScreenRecord", "UatTestCase", "Deployment", "Defect"],
    integrations: ["QA Deployment Registry", "Deployment Registry", "UAT Testing", "Ask Buddy"],
    lastUpdated: "2026-08-20",
    lastChange: "Phase 1 non-destructive QA, UAT, and Deployment workspace hub added.",
  },
  "/guided-onboarding": {
    pageTitle: "Guided Onboarding",
    pageIcon: "📚",
    description: "Single entry point for the preserved guided onboarding sequence and its underlying step routes.",
    features: ["Guided Learning", "Existing Capability Review", "Discovery Training"],
    apis: ["Existing onboarding routes"],
    stories: ["Complete Guided Onboarding"],
    screens: ["Guided Onboarding Hub", "Onboarding Steps 1–7"],
    businessRules: ["Underlying onboarding steps remain preserved", "Guided onboarding is classified as Historical / Training rather than standard operating navigation"],
    batches: [],
    businessObjects: ["OnboardingProgress", "DiscoveryRequirement"],
    integrations: ["Learning Center", "Ask Buddy"],
    lastUpdated: "2026-08-20",
    lastChange: "Phase 1 single-entry guided onboarding route added; underlying steps preserved.",
  },
