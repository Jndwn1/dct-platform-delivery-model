...
export default function RogerScreenTestingStatus() {
  ...
  const registryGridColumns = "minmax(112px, 1.15fr) minmax(96px, 0.92fr) minmax(140px, 1.75fr) 72px 72px 72px minmax(90px, 0.95fr) minmax(150px, 1.45fr) 54px";
  ...
  return (
    <section style={{ backgroundColor: "#ffffff", border: "1px solid #cbd5e1", borderRadius: "10px", overflow: "hidden", marginTop: "28px" }}>
      ...
      <div style={{ padding: "16px", borderBottom: "1px solid #e2e8f0" }}>
        ...
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", minWidth: "760px", borderCollapse: "collapse", fontSize: "11px" }}><thead><tr style={{ backgroundColor: "#eef2f7" }}>{["Milestone", "Owner(s)", "Date", "Status / Notes"].map(head => <th key={head} style={{ textAlign: "left", padding: "8px", color: "#1e3a5f", fontSize: "9px", letterSpacing: "0.06em", textTransform: "uppercase" }}>{head}</th>)}</tr></thead><tbody>{ROGER_MVP_MILESTONES.map(milestone => <tr key={milestone.milestone} style={{ borderTop: "1px solid #e2e8f0" }}><td style={{ padding: "8px", color: "#0f172a", fontWeight: 700 }}>{milestone.milestone}</td><td style={{ padding: "8px", color: "#475569" }}>{milestone.owners}</td><td style={{ padding: "8px", color: milestone.date.toUpperCase() === "TBD" ? "#92400e" : "#166534", fontWeight: 800 }}>{milestone.date}</td><td style={{ padding: "8px", color: "#475569", lineHeight: 1.4 }}>{milestone.notes}</td></tr>)}</tbody></table></div>
      </div>

      <div style={{ overflowX: "hidden", width: "100%" }}>
        <div style={{ minWidth: 0, width: "100%" }}>
          <div style={{ display: "grid", gridTemplateColumns: registryGridColumns, gap: "6px", backgroundColor: "#0f1623", padding: "9px 10px" }}>{["Screen / Area", "Delivery / Current Status", "What's Not Working / Dependency", "Dev Ready", "QA Ready", "UAT Ready", "Owner / Dependency", "Notes", ""].map((head, index) => <div key={`${head}-${index}`} style={{ color: "#bfdbfe", fontSize: "8px", fontWeight: 800, letterSpacing: "0.045em", textTransform: "uppercase", overflowWrap: "anywhere" }}>{head}</div>)}</div>
          {visible.map((screen, index) => {
            ...
            return <div key={screen.id} style={{ display: "grid", gridTemplateColumns: registryGridColumns, gap: "6px", alignItems: "start", height: "104px", boxSizing: "border-box", overflow: "hidden", padding: "10px", borderBottom: "1px solid #e2e8f0", backgroundColor: editing ? "#eff6ff" : index % 2 ? "#f8fafc" : "#fff" }}>
              ...
              <div>{editing ? <><button onClick={save} style={{ display: "block", width: "100%", padding: "5px 2px", marginBottom: "4px", border: "none", borderRadius: "4px", background: "#059669", color: "#fff", fontSize: "10px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>Save</button><button onClick={() => { setEditingId(null); setDraft(null); }} style={{ display: "block", width: "100%", padding: "5px 2px", border: "1px solid #cbd5e1", borderRadius: "4px", background: "#fff", color: "#475569", fontSize: "10px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>Cancel</button></> : <button onClick={() => { setEditingId(screen.id); setDraft({ ...screen }); }} style={{ width: "100%", padding: "5px 2px", border: "1px solid #bfdbfe", borderRadius: "4px", background: "#eff6ff", color: "#1d4ed8", fontSize: "10px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>Edit</button>}</div>
            </div>;
          })}
