...
const DATA_DOMAINS: DataDomain[] = [
  {
    ...
    notes: "Implemented (PI 1 Complete). B2 established normalized financial facts with XLOB classification. B2A enforced FirmTaxonomyId contract — records without classification are now rejected. PeriodStart/PeriodEnd correct. TaxYear correctly excluded from PDC.",
  },
  {
    id: "D02",
    ...
    notes: "Implemented (PI 1 Complete). B2A contract enforcement layer is live. FirmTaxonomyId is required on every normalized record — records without it are rejected in full, no partial acceptance. Classification results failing contract validation are rejected with a locked rejection code. Validation audit log is queryable.",
  },
  {
    id: "D03",
  ...
  },
  {
    ...
    notes: "Implemented (PI 2 Complete). B5 closed the EntityId risk open since PI 1. PDC is now the authoritative entity registry with GUID-based immutable EntityIds. Ownership hierarchies, jurisdiction data, and entity characteristics are loaded via governed Admin API. DataSourceType field recorded on every entity record. CEM sync is live and idempotent. User entitlement mappings stored and retrievable.",
  },
  {
    id: "D05",
  ...
  },
  {
    ...
    notes: "Implemented (PI 2 Complete). B6 delivered the full practitioner review workflow. Review tasks are generated automatically from data state via confidence band thresholds. Book-to-tax adjustments move through a governed six-state lifecycle (DRAFT → SUBMITTED → APPROVED → APPLIED → LOCKED). Tax-ready records are derived deterministically. Sign-off includes SHA-256 hash over the complete signed dataset. Lock is terminal at the record level.",
  },
  {
    id: "D08",
    ...
    notes: "In Review (PI 2 Active). ARCHITECTURAL CHANGE: B9 repurposed from IMS Integration & Prior Year Retrieval to Roger Gateway & Governed Consumer Access Layer (surface-not-store). Ocelot gateway scaffolding with auth and routing is in review. IMS, CEM, and TIM pass-through surfaces are gated on downstream system contract readiness. eODS deferred. Gateway Read Contract publication is the final gate. Roger and all consumers must call the gateway — not underlying systems directly.",
  },
  {
    id: "D09",
