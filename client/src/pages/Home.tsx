...
const BATCH_CALENDAR_PI23 = [
  ...
  { pi: "PI 2", status: "Done",        batch: "B10",   feat: "TDC",     name: "Return Assembly, Filing & Lineage",                                 startDate: "Wed 6/3",   endDate: "Sun 9/21",  whatItDoes: "Assembles the return, creates the immutable filing record, anchors lineage.",                                                                    rogerImpact: "Form 1120 / filing (stage 10)" },
...
const BATCH_REFERENCE = [
  ...
  { pi: "PI 2", status: "In Progress", batchNum: "7", platform: "TDC", name: "Client Tax Profile & Eligibility", whatItDoes: "Holds the client tax profile and determines which rules apply.", rogerImpact: "Eligibility" },
  ...
  { pi: "PI 2", status: "In Progress", batchNum: "42", platform: "TDC", name: "Tax Rules Framework & Book-to-Tax Adjustment Rules", whatItDoes: "Computes book-to-tax adjustments using governed rules.", rogerImpact: "Tax Adjustment (Stage 7) + Rule Administration" },
  ...
  { pi: "PI 3", status: "In Progress", batchNum: "MVP", platform: "TDC",     name: "MVP Enhancements",  whatItDoes: "Delivers the current Rule Logic Expression Table and Adjustment Subtype enhancements for the MVP experience.",               rogerImpact: "Filing Review + Adjustments screen + Master Workbook import" },
  { pi: "PI 3", status: "Done",        batchNum: "17", platform: "PDC",     name: "Many-to-Many Form Line Mapping & Jurisdiction-Aware Derivation",     whatItDoes: "Supports many-to-many form line mappings with jurisdiction-aware tax derivation.",                                                              rogerImpact: "Line Mappings (Stage 2) + Jurisdiction" },
...
export default function Home() {
  ...
  const piStatusRows = useMemo(() => {
    ...
    return rows.map(({ label, metric }) => {
      ...
      return {
        label,
        status: complete ? "Complete" : `Active · ${metric.complete}/${metric.total}`,
        color: complete ? "#059669" : "#2563eb",
        bg: complete ? "#f0fdf4" : "#eff6ff",
        border: complete ? "#bbf7d0" : "#bfdbfe",
      };
    });
  }, [piCompletion]);
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>(() => {
    ...
    return Object.fromEntries(ACCORDION_IDS.map(id => [id, false]));
  });
      ...
      <div style={{
        ...
        <div style={{ marginTop: "24px", position: "relative" }}>
          ...
          <div style={{ height: "12px", backgroundColor: "rgba(255,255,255,0.1)", borderRadius: "6px", overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${overallPct}%`, background: "linear-gradient(90deg, #059669, #34d399)", borderRadius: "6px", transition: "width 0.5s ease", boxShadow: "0 0 8px rgba(52,211,153,0.5)" }} />
          </div>
        </div>
          ...
          <div style={{ backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "12px 16px" }}>
            ...
            {piStatusRows.map(pi => (
              <div key={pi.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "6px" }}>
                ...
                <span style={{
                  fontSize: "10px", fontWeight: 700, color: pi.color,
                  backgroundColor: pi.bg, border: `1px solid ${pi.border}`,
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }}>
          ...
          <div style={{ backgroundColor: "#eff6ff", borderRadius: "8px", padding: "14px 16px", borderLeft: "3px solid #2563eb" }}>
            <div style={{ fontSize: "11px", fontWeight: 700, color: "#1e3a5f", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "8px" }}>🔵 Active ADO Workstreams ({deliveryHighlightActiveBatchFeatures.length})</div>
