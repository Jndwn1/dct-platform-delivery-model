...
const BATCH_CONTENT: Record<string, BatchContent> = {
  ...
  B9: {
    gate: "G4 — Lineage Closure",
    systems: ["PDC", "IMS", "CEM", "TIM", "Roger UI"],
    lead: "PDC Workstream Lead",
    entryCondition: "Batch 8 PDC complete; downstream system (IMS, CEM, TIM) contract readiness confirmed",
    exitCondition: "Ocelot gateway deployed as single consumer entry point; IMS/CEM/TIM pass-through surfaces live; Gateway Read Contract published; Roger calling gateway — not underlying systems directly",
    stories: [
      { title: "Ocelot Gateway Scaffolding & Governed Consumer Access Layer", wmbt: "Gateway scaffolding deployed with auth and routing. Published as the single entry point for Roger and all consumers. No consumer calls underlying systems directly." },
      { title: "IMS Pass-Through Surface", wmbt: "Prior year and current year tax return data available on demand via IMS pass-through. Data is not stored in PDC — surfaced only." },
      { title: "CEM Pass-Through Surface", wmbt: "Client authorization and client-to-user mapping available via CEM pass-through. Not stored in PDC." },
      { title: "TIM Pass-Through Surface", wmbt: "Engagement metadata, deliverables, due dates, engagement acceptance & continuance records available via TIM pass-through. Not stored in PDC." },
      { title: "Gateway Read Contract Publication (Roger Consumer Surface)", wmbt: "Gateway Read Contract published as the versioned consumer surface for Roger. Contract is additive-only once published. eODS pass-through deferred — can layer in later without changing the gateway contract." },
    ],
  },
  B10: {
    gate: "G4 — Lineage Closure",
