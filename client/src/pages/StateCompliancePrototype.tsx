...
function StateFilerRow({ filer, expanded, onToggle, onOpen }: { filer: StateFiler; expanded: boolean; onToggle: () => void; onOpen: (filer: StateFiler) => void }) {
  ...
  return <>
    ...
    {expanded && filer.filings.map(filing => <tr key={`${filer.id}-${filing.code}`} style={{ background: "#fbfdfe" }}><td style={{ padding: "9px 12px 9px 42px", borderBottom: "1px solid #edf1f4" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ display: "grid", width: 20, height: 17, placeItems: "center", borderRadius: 3, background: "#e8f3f8", color: "#0878aa", fontSize: 9, fontWeight: 800 }}>{filing.code}</span>{filing.type === "Income" ? <button onClick={() => onOpen(filer)} aria-label={`Open State Compliance workflow for ${filing.state} Income Tax Return`} style={{ border: 0, padding: 0, color: "#0878aa", background: "transparent", fontWeight: 800, fontSize: 11, textDecoration: "underline", textUnderlineOffset: 2, cursor: "pointer" }}>{filing.state} State Income Tax Return</button> : <strong>{filing.state} State {filing.type} Return</strong>}<span style={{ color: "#94a3b8", fontSize: 10 }}>TIM source</span></div></td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}>{filing.type}</td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}><strong>{filing.form}</strong></td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}><StateStatus status={filing.status} /></td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}><ProgressBar value={filing.progress} color={filing.status === "Review" ? STYLE.green : STYLE.blue} /></td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}>{filing.statutoryDue}</td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}>{filing.clientDue}</td><td style={{ padding: "9px 12px", borderBottom: "1px solid #edf1f4" }}><ActionButton onClick={() => onOpen(filer)} variant="text">Open</ActionButton></td></tr>)}
  </>;
}
