...
const STATE_STORIES: DiscoveryStory[] = [
  {
    ...
    outcome: "Roger receives the approved, read-only State return-filing response for the first filing screen. The story is refined and DCT owns governed backend delivery.",
    ...
    owner: "DCT",
    dct: "DCT owns delivery of the refined Gateway composition and manages the TIM/Roger contract and workflow dependencies through PI4 planning.",
    roger: "Consume the governed response and implement the approved State Filing-screen representation without owning State tax records.",
    questions: ["Maintain the authoritative State filing-group relationship as an implementation dependency.", "Maintain the State Issue Count definition and final Roger workflow representation as contract inputs to DCT delivery."],
    dependencies: ["Authoritative TIM State filing-group relationship", "TIM endpoint and field-level data contract", "Roger State UI and workflow decision", "State Issue Count definition"],
    existing: "B9A Gateway governed access and B16 audit / lineage patterns.",
    netNew: "Authoritative State filing-group relationship and a Roger-ready State Filing response contract.",
    implementation: "DCT accepted the refined implementation scope. Coordinate the TIM relationship, State Issue Count, and Roger workflow contract as managed delivery dependencies; do not assign a PI4 sprint until the baseline is approved.",
    estimation: "Ready with Dependency",
    acceptance: "Ready",
    reason: "Refinement is complete and DCT has accepted ownership. TIM/Roger relationship and workflow dependencies remain managed delivery inputs rather than blockers to ownership.",
  },
  {
    id: "1471480 – DCT — Retrieve Return Structure Starting Context for a State Filer / Filing Group",
    outcome: "Roger starts a State filing group from an available Federal filing group and enables permitted practitioner adjustment when State membership differs. The story is refined and DCT owns governed backend delivery.",
    ...
    owner: "DCT",
    dct: "DCT owns delivery of the refined retrieval, persistence, and governance behavior and manages the TIM relationship and State decision dependencies through PI4 planning.",
    roger: "Use the available Federal filing group as the State starting point and present the approved practitioner adjustment experience.",
    questions: ["Maintain the authoritative TIM parent/child and State filing-group relationship as an implementation dependency.", "Maintain the Combined/Unitary selection and persistence decision as a managed delivery input."],
    dependencies: ["Authoritative TIM parent/child and State filing-group relationship", "TIM endpoint, grouping identifier, and field contract", "Roger State filing-group workflow", "Combined / Unitary business and persistence decision"],
    existing: "B9A Gateway governed access, TDC retrieval patterns, and B16 audit / lineage patterns.",
    netNew: "State filer / filing-group starting-context contract and authoritative grouping retrieval logic.",
    implementation: "DCT accepted the refined implementation scope. Coordinate the TIM endpoint, grouping identifier, State Issue Count, and Combined/Unitary decisions as managed delivery dependencies; do not assign a PI4 sprint until the baseline is approved.",
    estimation: "Ready with Dependency",
    acceptance: "Ready",
    reason: "Refinement is complete and DCT has accepted ownership. Remaining TIM and State workflow decisions are managed delivery inputs for the DCT implementation plan.",
  },
];
...
const STATE_REFINEMENT_ROWS = [
  {
    ...
    dependency: "Refinement is complete and DCT owns delivery. The authoritative State filing-group relationship, State Issue Count, and final Roger workflow representation remain managed implementation dependencies in the PI4 plan.",
  },
  {
    ...
    dependency: "Refinement is complete and DCT owns delivery. The TIM parent/child relationship, endpoint, grouping identifier, State Issue Count, and Combined/Unitary decision remain managed implementation dependencies in the PI4 plan.",
  },
] as const;

const STATE_DELIVERY_DEPENDENCIES = [
  { title: "TIM State Filing-Group Relationship", items: ["Maintain the relationship that ties related State deliverables to the correct filer or filing group.", "Maintain the authoritative grouping identifier and TIM relationship as a DCT delivery dependency."] },
  { title: "TIM → Roger/DCT Data Contract", items: ["Manage the authoritative TIM endpoint and exact fields Roger/DCT will receive.", "Maintain the parent/child, jurisdiction, filer, designation, and filing-group contract through implementation planning."] },
  { title: "Roger State UI / Workflow", items: ["Manage the approved Roger filing and filing-group representation as a consumer contract input.", "Maintain the Federal filing-group starting point and the Gateway response needed to support the final UI."] },
  { title: "State Issue Count & Combined / Unitary", items: ["Manage the State Issue Count source and calculation rules as a delivery dependency.", "Manage the Combined/Unitary selection, persistence location, and downstream use where TIM does not provide it."] },
] as const;

function StateRefinementReadiness() {
  const copyText = STATE_REFINEMENT_ROWS.map((row) => [
    row.story,
    "Refinement status: Refined — DCT owned",
    "What was clarified: " + row.clarified,
    "Managed delivery dependencies: " + row.dependency,
  ].join("\n")).join("\n\n");

  return <div style={{ marginTop: "16px" }}>
    <div style={{ backgroundColor: "#ffffff", border: "1px solid #bbf7d0", borderRadius: "10px", overflow: "hidden" }}>
      <div style={{ padding: "13px 15px", borderBottom: "1px solid #bbf7d0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}><div><div style={{ color: C.navy, fontSize: "12px", fontWeight: 800 }}>State Story Refinement &amp; Ownership</div><div style={{ color: "#047857", fontSize: "10px", marginTop: "2px" }}>Both approved State stories are refined and DCT-owned; PI4 sprints remain unassigned pending baseline approval.</div></div><CopyButton text={copyText} label="Copy State matrix" /></div>
      <div style={{ overflowX: "auto" }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: "1100px", fontSize: "11px" }}><thead><tr style={{ backgroundColor: C.navy }}>{["Story", "Status After Refinement", "What Was Clarified", "Managed Delivery Dependencies"].map(header => <th key={header} style={{ color: "#ffffff", textAlign: "left", padding: "11px 12px", fontWeight: 750, minWidth: header === "Story" ? "260px" : "230px" }}>{header}</th>)}</tr></thead><tbody>{STATE_REFINEMENT_ROWS.map((row, index) => <tr key={row.story} style={{ backgroundColor: index % 2 === 0 ? "#f0fdf4" : "#ffffff", borderBottom: "1px solid #dcfce7" }}><td style={{ padding: "12px", verticalAlign: "top", color: C.navy, fontWeight: 800, lineHeight: "1.45" }}>{row.story}</td><td style={{ padding: "12px", verticalAlign: "top" }}><span style={{ display: "inline-flex", alignItems: "center", backgroundColor: "#ecfdf5", border: "1px solid #a7f3d0", color: "#047857", borderRadius: "999px", padding: "4px 8px", fontSize: "10px", fontWeight: 800, whiteSpace: "nowrap" }}>✓ Refined · DCT owned</span></td><td style={{ padding: "12px", verticalAlign: "top", color: "#334155", lineHeight: "1.5" }}>{row.clarified}</td><td style={{ padding: "12px", verticalAlign: "top", color: "#166534", lineHeight: "1.5" }}>{row.dependency}</td></tr>)}</tbody></table></div>
    </div>
    <section aria-label="Managed State Delivery Dependencies" style={{ marginTop: "16px", backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "10px", padding: "16px" }}><div style={{ color: "#1d4ed8", fontSize: "12px", fontWeight: 800, marginBottom: "4px" }}>Managed State Delivery Dependencies</div><p style={{ color: "#1e3a8a", fontSize: "10px", lineHeight: "1.45", margin: "0 0 12px" }}>DCT owns implementation for both refined stories. These dependencies remain visible for contract management, sequencing, and evidence during PI4 planning.</p><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "10px" }}>{STATE_DELIVERY_DEPENDENCIES.map((item, index) => <div key={item.title} style={{ backgroundColor: "#ffffff", border: "1px solid #bfdbfe", borderLeft: "4px solid #2563eb", borderRadius: "7px", padding: "11px" }}><div style={{ color: "#1d4ed8", fontSize: "11px", fontWeight: 800, marginBottom: "6px" }}>{index + 1}. {item.title}</div><ul style={{ margin: 0, paddingLeft: "16px" }}>{item.items.map(detail => <li key={detail} style={{ color: "#1e3a8a", fontSize: "10px", lineHeight: "1.45", marginBottom: "4px" }}>{detail}</li>)}</ul></div>)}</div></section>
    <section aria-label="DCT Delivery Guardrails" style={{ marginTop: "16px", backgroundColor: "#f0fdf4", border: "1px solid #bbf7d0", borderLeft: "5px solid #059669", borderRadius: "10px", padding: "15px 16px" }}><div style={{ color: "#047857", fontSize: "12px", fontWeight: 800, marginBottom: "6px" }}>DCT Delivery Guardrails</div><p style={{ color: "#166534", fontSize: "12px", lineHeight: "1.55", margin: "0 0 8px" }}>DCT owns the backend delivery for both refined stories. Dependency decisions remain tracked as implementation inputs so the governed contracts, QA evidence, and lineage requirements stay explicit.</p><p style={{ color: "#166534", fontSize: "11px", lineHeight: "1.5", margin: 0 }}>This ownership update does not assign a PI4 sprint or change delivery metrics; sprint commitments begin only after PI4 baseline approval.</p></section>
    <section aria-label="State Ownership Decision" style={{ marginTop: "16px", backgroundColor: "#ffffff", border: "1px solid #bbf7d0", borderRadius: "10px", padding: "15px 16px" }}><div style={{ color: C.navy, fontSize: "12px", fontWeight: 800, marginBottom: "8px" }}>Ownership Decision</div><div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px" }}><span style={{ color: "#047857", fontSize: "11px", fontWeight: 800 }}>1472734 – ✓ Refined · DCT owned</span><span style={{ color: "#047857", fontSize: "11px", fontWeight: 800 }}>1471480 – ✓ Refined · DCT owned</span></div><p style={{ color: "#475569", fontSize: "11px", lineHeight: "1.55", margin: 0 }}>DCT has taken ownership of both refined State stories. Manage the remaining State, TIM, and Roger dependencies through the PI4 implementation plan while keeping sprint assignment and delivery metrics unchanged until the baseline is approved.</p></section>
  </div>;
}
...
function WorkstreamReadinessHub({ active, onChange }: { active: "state" | "provision"; onChange: (workstream: "state" | "provision") => void }) {
  ...
  const overview = isState ? "State refinement is complete for two DCT-owned stories: 1472734 for the read-only Gateway return-filing response for Roger and 1471480 for Return Structure starting context for a State filer or filing group. State, TIM, and Roger items are managed implementation dependencies; PI4 sprint assignment remains pending baseline approval." : "Package 1 Return-to-Provision Review compares prior-year Provision amounts with the prior-year filed Tax Return by Description and entity. RTP Difference = PY Tax Return − PY Provision, and each difference carries into the current-year provision true-up.";
  ...
  const risks = isState ? ["1472734 is DCT-owned; manage the authoritative State filing-group relationship, State Issue Count definition, and final Roger workflow representation as delivery dependencies for the Gateway response contract.", "1471480 is DCT-owned; manage the TIM parent/child and filing-group relationship, endpoint, required fields, grouping identifier, and Combined/Unitary decision as delivery dependencies for retrieval and persistence."] : ["PY correction semantics are a major unresolved accounting decision.", "Package 2 / Package 3 routing needs detailed business rules before downstream acceptance."];
  const decisionItems = isState ? ["Manage the TIM State filing-group relationship and Roger/Gateway data contract for 1472734.", "Manage the TIM endpoint, grouping identifier, and State Return Structure starting-context contract for 1471480."] : ["Decide whether a correction changes source values or creates an override.", "Decide routing for Permanent / Temporary, payable, and deferred outputs."];
...
const STATE_STORY_READINESS = [
  "Both refined State stories are DCT-owned delivery items.",
  "Detailed requirements remain directly in each story so Development can build and QA can test without inference.",
  "The State, TIM, and Roger contract inputs remain visible as managed delivery dependencies.",
  ...
  "DCT manages implementation design, governed contracts, and evidence for the refined State stories.",
  "PI4 sprint assignment remains pending the approved baseline; ownership does not change delivery metrics.",
] as const;
    ...
    <section id="pi4-state-readiness" style={{ marginBottom: "48px" }}>
      ...
      <div style={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "10px", overflow: "hidden", marginBottom: "24px" }}>
        <div style={{ padding: "15px 20px", borderBottom: "1px solid #e2e8f0" }}>
          <div style={{ fontSize: "11px", fontWeight: 800, color: C.navy, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "4px" }}>Current State Story Refinement &amp; Ownership</div>
          <p style={{ fontSize: "12px", color: "#047857", margin: 0, lineHeight: "1.5" }}>Both approved State stories are refined and DCT-owned. State, TIM, and Roger items remain managed implementation dependencies; no PI4 sprint or delivery metric is assigned until the baseline is approved.</p>
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", minWidth: "900px", borderCollapse: "collapse", fontSize: "12px" }}>
            <thead><tr style={{ backgroundColor: C.navy }}>{["Story", "Scope", "Refinement / Ownership Status", "Delivery Assessment"].map(label => <th key={label} style={{ padding: "11px 14px", textAlign: "left", color: "#ffffff", fontWeight: 750, fontSize: "11px" }}>{label}</th>)}</tr></thead>
