...
const STATE_PRIMARY_QUESTIONS: Record<string, string> = {
  "1472734": "Can State, TIM, and Roger confirm the authoritative State filing-group relationship, the required Roger workflow representation, and the fields Gateway must compose for the read-only Filing screen?",
  "1471480": "Can State, TIM, and Roger confirm the authoritative parent/child and filing-group relationship, TIM endpoint, grouping identifier, and fields required to retrieve Return Structure starting context?",
};

const STATE_STORIES: DiscoveryStory[] = [
  {
    id: "1472734 – DCT Gateway — Compose State Return-Filing Response for Roger",
    outcome: "Roger receives the approved, read-only State return-filing response for the first filing screen after the authoritative State filing-group relationship and UI workflow are confirmed.",
    scope: "Compose the governed Gateway response for the first read-only State Filing screen using TIM-provided filer, State activation, deliverables, Single/Consolidated indicator, and Deliverable Due Date.",
    owner: "State Business Team", dct: "Define and implement the Gateway composition only after the business workflow and authoritative TIM relationship and field contract are confirmed.", roger: "Finalize the State Filing-screen representation and consume the governed response without owning State tax records.",
    questions: [STATE_PRIMARY_QUESTIONS["1472734"], "Confirm how multiple State deliverables relate to the correct filer and filing group, including the authoritative grouping identifier.", "Define State Issue Count, including what constitutes an issue, its authoritative source, and calculation rules.", "Finalize the Roger/UI State workflow representation because it affects the Gateway response contract."],
    dependencies: ["Authoritative TIM State filing-group relationship", "TIM endpoint and field-level data contract", "Roger State UI and workflow decision", "State Issue Count definition"], existing: "B9A Gateway governed access and B16 audit / lineage patterns.", netNew: "Authoritative State filing-group relationship and a Roger-ready State Filing response contract.", implementation: "TIM currently supplies the filer, State activation, deliverables, Single/Consolidated indicator, and Deliverable Due Date. DCT must not infer grouping logic or finalize the Gateway response contract before the State and Roger decisions are confirmed.", estimation: "Not Ready", acceptance: "Not Ready", reason: "The authoritative State filing-group relationship, State Issue Count, and final Roger workflow representation remain unresolved and materially change the response DCT must build.",
  },
  {
    id: "1471480 – DCT — Retrieve Return Structure Starting Context for a State Filer / Filing Group",
    outcome: "Roger starts a State filing group from an available Federal filing group and enables permitted practitioner adjustment when State membership differs.",
    scope: "Retrieve authoritative Return Structure starting context for a State filer or filing group, including parent/child and member relationships required by the approved State workflow.",
    owner: "State Business Team", dct: "Define retrieval logic only after TIM identifies the authoritative parent/child, State deliverable, and filing-group relationship and required fields.", roger: "Use the available Federal filing group as the State starting point and present the approved practitioner adjustment experience.",
    questions: [STATE_PRIMARY_QUESTIONS["1471480"], "Confirm how TIM ties entities and State deliverables to a specific State filing group and whether it provides the relationship directly.", "Confirm the exact TIM endpoint, required fields, grouping identifier, and authoritative parent/child relationship for Roger/DCT.", "Confirm how Combined/Unitary is selected, persisted, and used downstream when it is not supplied by TIM."],
    dependencies: ["Authoritative TIM parent/child and State filing-group relationship", "TIM endpoint, grouping identifier, and field contract", "Roger State filing-group workflow", "Combined / Unitary business and persistence decision"], existing: "B9A Gateway governed access, TDC retrieval patterns, and B16 audit / lineage patterns.", netNew: "State filer / filing-group starting-context contract and authoritative grouping retrieval logic.", implementation: "The sample TIM response does not establish the relationship required to retrieve a State filing group. Roger currently uses a lighter TIM response; available additional data points are not yet being pulled for Roger. DCT must not define retrieval logic until the authoritative relationship and required fields are confirmed.", estimation: "Not Ready", acceptance: "Not Ready", reason: "The TIM endpoint, required fields, grouping identifier, and authoritative filing-group relationship are not confirmed; these decisions materially change retrieval logic and estimate scope.",
  },
];
...
const STATE_REFINEMENT_ROWS = [
  {
    story: "1472734 – DCT Gateway — Compose State Return-Filing Response for Roger",
    clarified: "The first State Filing screen is read-only. TIM provides the filer, State activation, deliverables, and the Single/Consolidated indicator. Deliverable Due Date is the source for the date shown on the screen.",
    open: "State filing-group relationship remains unresolved. TIM does not yet provide the relationship needed to reliably group multiple State deliverables to the correct filer or filing group. State Issue Count is undefined, including what qualifies as an issue and how it is calculated. The Roger/UI State workflow representation must also be finalized because it affects the Gateway response contract.",
  },
  {
    story: "1471480 – DCT — Retrieve Return Structure Starting Context for a State Filer / Filing Group",
    clarified: "The State filing group begins with an available Federal filing group, and the practitioner may adjust member entities when the State filing group differs. Filing group means the entities or members under the parent, not the Return Structure itself.",
    open: "The authoritative TIM parent/child and filing-group relationship is not defined. The sample TIM response does not show how entities or State deliverables tie to a specific State filing group. Roger currently uses a lighter TIM response, and available additional data points are not currently pulled for Roger. The exact TIM endpoint, required fields, grouping identifier, and authoritative relationship must be confirmed before DCT can define retrieval logic and estimate the work.",
  },
] as const;

const STATE_REFINEMENT_OPEN_ITEMS = [
  { title: "TIM State Filing-Group Relationship", items: ["Confirm how related State deliverables are tied to the correct filer or filing group.", "Identify the authoritative grouping identifier.", "Confirm whether TIM will provide this relationship directly.", "DCT should not create inferred or temporary Gateway grouping logic unless the business and architecture teams explicitly approve that approach."] },
  { title: "TIM → Roger/DCT Data Contract", items: ["Confirm the authoritative TIM endpoint.", "Confirm the exact fields Roger/DCT will receive.", "Confirm how parent/child relationships are represented.", "Confirm how State, jurisdiction, filer, filing designation, and filing-group relationships are identified."] },
  { title: "Roger State UI / Workflow", items: ["Confirm how State filings and filing groups will be represented in Roger.", "Confirm how the practitioner selects and enters a State filing workflow.", "Confirm how Federal filing-group membership is used as the starting point for State.", "Confirm what information Gateway must provide to support the final UI."] },
  { title: "State Issue Count", items: ["Define what constitutes an issue for State.", "Determine whether State Issue Count follows the Federal model or requires separate logic.", "Identify the authoritative source and calculation rules."] },
  { title: "Combined / Unitary Handling", items: ["TIM provides the Single/Consolidated indicator, but Combined/Unitary is not currently available from TIM.", "Confirm whether the practitioner will select Combined/Unitary in Roger.", "Confirm where that selection is persisted and how it is used downstream."] },
] as const;

function StateRefinementReadiness() {
  const copyText = STATE_REFINEMENT_ROWS.map(row => `${row.story}\nReadiness after refinement: Not Ready to Estimate\nWhat was clarified: ${row.clarified}\nStill open: ${row.open}`).join("\n\n");
  return <div style={{ marginTop: "16px" }}>
    <div style={{ backgroundColor: "#ffffff", border: "1px solid #fecaca", borderRadius: "10px", overflow: "hidden" }}>
      <div style={{ padding: "13px 15px", borderBottom: "1px solid #fecaca", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}><div><div style={{ color: C.navy, fontSize: "12px", fontWeight: 800 }}>State Story Readiness Matrix</div><div style={{ color: "#991b1b", fontSize: "10px", marginTop: "2px" }}>Latest State refinement assessment — only the two State stories in scope are shown.</div></div><CopyButton text={copyText} label="Copy State matrix" /></div>
      <div style={{ overflowX: "auto" }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: "1100px", fontSize: "11px" }}><thead><tr style={{ backgroundColor: C.navy }}>{["Story", "Readiness After Refinement", "What Was Clarified", "What Is Still Open / Why It Is Not Ready to Estimate"].map(header => <th key={header} style={{ color: "#ffffff", textAlign: "left", padding: "11px 12px", fontWeight: 750, minWidth: header === "Story" ? "260px" : "230px" }}>{header}</th>)}</tr></thead><tbody>{STATE_REFINEMENT_ROWS.map((row, index) => <tr key={row.story} style={{ backgroundColor: index % 2 === 0 ? "#fef2f2" : "#ffffff", borderBottom: "1px solid #fee2e2" }}><td style={{ padding: "12px", verticalAlign: "top", color: C.navy, fontWeight: 800, lineHeight: "1.45" }}>{row.story}</td><td style={{ padding: "12px", verticalAlign: "top" }}><span style={{ display: "inline-flex", alignItems: "center", backgroundColor: "#fef2f2", border: "1px solid #fecaca", color: "#b91c1c", borderRadius: "999px", padding: "4px 8px", fontSize: "10px", fontWeight: 800, whiteSpace: "nowrap" }}>🔴 Not Ready to Estimate</span></td><td style={{ padding: "12px", verticalAlign: "top", color: "#334155", lineHeight: "1.5" }}>{row.clarified}</td><td style={{ padding: "12px", verticalAlign: "top", color: "#7f1d1d", lineHeight: "1.5" }}>{row.open}</td></tr>)}</tbody></table></div>
    </div>
    <section aria-label="Open Items from State Refinement" style={{ marginTop: "16px", backgroundColor: "#fff7ed", border: "1px solid #fed7aa", borderRadius: "10px", padding: "16px" }}><div style={{ color: "#9a3412", fontSize: "12px", fontWeight: 800, marginBottom: "4px" }}>Open Items from State Refinement</div><p style={{ color: "#7c2d12", fontSize: "10px", lineHeight: "1.45", margin: "0 0 12px" }}>These business, workflow, and upstream data-contract decisions require confirmation before DCT can define the requested State behavior.</p><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "10px" }}>{STATE_REFINEMENT_OPEN_ITEMS.map((item, index) => <div key={item.title} style={{ backgroundColor: "#ffffff", border: "1px solid #fed7aa", borderLeft: "4px solid #ea580c", borderRadius: "7px", padding: "11px" }}><div style={{ color: "#9a3412", fontSize: "11px", fontWeight: 800, marginBottom: "6px" }}>{index + 1}. {item.title}</div><ul style={{ margin: 0, paddingLeft: "16px" }}>{item.items.map(detail => <li key={detail} style={{ color: "#7c2d12", fontSize: "10px", lineHeight: "1.45", marginBottom: "4px" }}>{detail}</li>)}</ul></div>)}</div></section>
    <section aria-label="Why DCT Cannot Estimate Yet" style={{ marginTop: "16px", backgroundColor: "#fef2f2", border: "1px solid #fecaca", borderLeft: "5px solid #dc2626", borderRadius: "10px", padding: "15px 16px" }}><div style={{ color: "#991b1b", fontSize: "12px", fontWeight: 800, marginBottom: "6px" }}>Why DCT Cannot Estimate Yet</div><p style={{ color: "#7f1d1d", fontSize: "12px", lineHeight: "1.55", margin: "0 0 8px" }}>These stories are not blocked because DCT needs final technical implementation details before estimating. They are not ready because unresolved business, workflow, and upstream data-contract decisions materially change what DCT would need to build.</p><ul style={{ margin: "0 0 8px", paddingLeft: "17px" }}>{["How State filings are authoritatively grouped.", "Which TIM endpoint and fields support the workflow.", "How the Roger State workflow represents the filing relationship.", "What Gateway must compose versus what comes directly from TIM.", "How State Issue Count is defined and calculated."].map(item => <li key={item} style={{ color: "#7f1d1d", fontSize: "11px", lineHeight: "1.5", marginBottom: "3px" }}>{item}</li>)}</ul><p style={{ color: "#7f1d1d", fontSize: "11px", lineHeight: "1.5", margin: 0 }}>Estimating before these decisions are resolved would require DCT to make assumptions about scope and solution design.</p></section>
    <section aria-label="State Readiness Decision" style={{ marginTop: "16px", backgroundColor: "#ffffff", border: "1px solid #fecaca", borderRadius: "10px", padding: "15px 16px" }}><div style={{ color: C.navy, fontSize: "12px", fontWeight: 800, marginBottom: "8px" }}>Readiness Decision</div><div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px" }}><span style={{ color: "#b91c1c", fontSize: "11px", fontWeight: 800 }}>1472734 – 🔴 Not Ready to Estimate</span><span style={{ color: "#b91c1c", fontSize: "11px", fontWeight: 800 }}>1471480 – 🔴 Not Ready to Estimate</span></div><p style={{ color: "#475569", fontSize: "11px", lineHeight: "1.55", margin: "0 0 7px" }}>DCT can continue reviewing the stories and identifying technical considerations, but the stories should not yet be accepted for DCT ownership or estimated until the remaining State, TIM, and Roger questions are resolved and the stories are updated with those decisions.</p><p style={{ color: "#475569", fontSize: "11px", lineHeight: "1.55", margin: 0 }}>The next readiness review should occur after the State and Roger teams confirm the TIM filing-group relationship, State UI workflow, and required data contract.</p></section>
  </div>;
}
...
function ProvisionPrototypeReference() {
  return <section aria-label="Provision Prototype" style={{ marginTop: "16px", marginBottom: "16px", backgroundColor: "#faf5ff", border: "1px solid #e9d5ff", borderLeft: `5px solid ${C.purple}`, borderRadius: "10px", padding: "16px 18px" }}>
    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
      <div style={{ minWidth: "260px", flex: 1 }}>
        <div style={{ color: C.purple, fontSize: "10px", fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "6px" }}>Provision Prototype</div>
        <a href={PROVISION_PROTOTYPE_URL} target="_blank" rel="noopener noreferrer" style={{ color: C.navy, fontSize: "15px", fontWeight: 800, lineHeight: "1.45", textDecoration: "none" }}>Roger — Tax Provision Prototype <span aria-hidden="true" style={{ color: C.purple }}>↗</span></a>
        <p style={{ color: "#475569", fontSize: "12px", lineHeight: "1.55", margin: "8px 0 0", maxWidth: "790px" }}>Interactive Roger Provision prototype used to illustrate the proposed Provision workflow, screen sequence, Return-to-Provision experience, Deferred Rollforward, and related practitioner interactions.</p>
      </div>
      <a href={PROVISION_PROTOTYPE_URL} target="_blank" rel="noopener noreferrer" style={{ alignSelf: "center", display: "inline-flex", alignItems: "center", justifyContent: "center", backgroundColor: C.purple, color: "#ffffff", borderRadius: "6px", padding: "9px 13px", fontSize: "11px", fontWeight: 800, textDecoration: "none", whiteSpace: "nowrap" }}>Open Provision Prototype</a>
    </div>
  </section>;
}
...
function WorkstreamReadinessHub({ active, onChange }: { active: "state" | "provision"; onChange: (workstream: "state" | "provision") => void }) {
  ...
  const overview = isState ? "State refinement is focused on two stories that are not ready to estimate: 1472734 for the read-only Gateway return-filing response for Roger and 1471480 for Return Structure starting context for a State filer or filing group. State, TIM, and Roger decisions must be confirmed before DCT accepts or estimates either story." : "Package 1 Return-to-Provision Review compares prior-year Provision amounts with the prior-year filed Tax Return by Description and entity. RTP Difference = PY Tax Return − PY Provision, and each difference carries into the current-year provision true-up.";
  ...
  const risks = isState ? ["1472734 requires the authoritative State filing-group relationship, State Issue Count definition, and final Roger workflow representation before the Gateway response contract can be defined.", "1471480 requires the authoritative TIM parent/child and filing-group relationship, endpoint, required fields, and grouping identifier before DCT can define retrieval logic and estimate scope."] : ["PY correction semantics are a major unresolved accounting decision.", "Package 2 / Package 3 routing needs detailed business rules before downstream acceptance."];
  const decisionItems = isState ? ["Confirm the TIM State filing-group relationship and Roger/Gateway data contract for 1472734.", "Confirm the TIM endpoint, grouping identifier, and State Return Structure starting-context contract for 1471480."] : ["Decide whether a correction changes source values or creates an override.", "Decide routing for Permanent / Temporary, payable, and deferred outputs."];
  return <section id="story-readiness" style={{ marginBottom: "48px" }}><SectionHeading title="State & Provision Story Readiness" subtitle="A consistent discovery structure that separates business ownership from DCT implementation responsibility." />
    ...
    {isState ? <StateRefinementReadiness /> : <><StoryMatrix stories={stories} /><ProvisionPrototypeReference /></>}
  </section>;
}
    ...
    <section id="pi4-state-readiness" style={{ marginBottom: "48px" }}>
      ...
      <div style={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "10px", overflow: "hidden", marginBottom: "24px" }}>
        <div style={{ padding: "15px 20px", borderBottom: "1px solid #e2e8f0" }}>
          ...
          <p style={{ fontSize: "12px", color: "#991b1b", margin: 0, lineHeight: "1.5" }}>Both approved State stories are not ready to estimate or accept for DCT ownership; no delivery size is inferred until the TIM filing-group relationship, State UI workflow, and required data contract are confirmed.</p>
        </div>
...
function ProvisionDeliveryReadiness() {
  ...
  return <section id="pi4-provision-readiness" style={{ marginBottom: "48px" }}>
    <SectionHeading title="Provision Delivery Readiness" subtitle="Package 1 planning view for Return-to-Provision readiness, refinement gates, and dependency sequencing." />
  ...
  return (
    <section id="s7" style={{ marginBottom: "48px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "16px" }}>
        <div style={{ flex: 1 }}>
          <SectionHeading title="Ask Buddy" subtitle="Before documenting any new requirement, ask Buddy to determine whether DCT already supports the capability. Buddy guides discovery — not just answers." />
        </div>
        <button
          onClick={clearChat}
