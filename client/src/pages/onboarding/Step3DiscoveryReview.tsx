...
const TOPICS = [
  ...
  },
  {
    ...
    content: [
      ...
      { label: "Step 5 — Gateway → GoSystem", desc: "GoSystem requests workpaper and provision exports from the Gateway. The Gateway returns GoSystem-scoped data in the correct export format." },
  ...
  },
  {
    ...
    color: "#be185d",
    content: [
      { label: "GoSystem's role", desc: "GoSystem is RSM's tax compliance and filing system. It receives governed workpaper and provision data from TDC via the Gateway and uses it for tax return preparation and regulatory filing." },
      { label: "What GoSystem receives", desc: "GoSystem receives: Tax workpapers, Provision schedules, Adjustment history, Audit trail exports, State apportionment data." },
      { label: "GoSystem does NOT", desc: "GoSystem does NOT feed data back into DCT. GoSystem does NOT make tax decisions. GoSystem is a downstream consumer only." },
      { label: "GoSystem export format", desc: "TDC generates GoSystem-compatible exports via Batch 28. The export format is fixed and must match the GoSystem import specification exactly." },
      { label: "Provision and State", desc: "Provision teams use GoSystem to prepare tax provisions using data received from TDC. State teams use GoSystem for state apportionment filings using TDC apportionment data." },
    ],
  },
];
